...
class Translator:
    ...
    # то новое русское слово добавляется как синоним для перевода, например, go - идти, ходить, ехать); 
    # если связка eng-rus уже существует, то второй раз ее добавлять не нужно, например:
    # add('go', 'идти'), add('go', 'идти');
    def add(self, eng, rus):
        # если нет такого ключа то добавляем ключ значение
        if eng not in self.d:
            self.d.update({eng: [rus]})

        else:
            # если ключ есть и есть такое значение то пропускаем действие
            if rus in tr.d[eng]:
                pass
            else:
...
# # car - машина
tr.add(*'car - машина'.split(' - '))
# # car - автомобиль
tr.add(*'car - автомобиль'.split(' - '))
# # leaf - лист
tr.add(*'leaf - лист'.split(' - '))
# # river - река
tr.add(*'river - река'.split(' - '))
# # go - идти
tr.add(*'go - идти'.split(' - '))
# # go - ехать
tr.add(*'go - ехать'.split(' - '))
# # go - ходить
tr.add(*'go - ходить'.split(' - '))
# # milk - молоко
tr.add(*'milk - молоко'.split(' - '))
