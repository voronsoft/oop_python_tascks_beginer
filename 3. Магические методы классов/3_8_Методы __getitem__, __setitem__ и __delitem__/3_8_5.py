...
создайте класс CellString для работы со строками и используйте тот же класс TableValues для этого нового типа данных.
...
class IntegerValue:  # дескриптор данных для работы с целыми числами.
    @classmethod
    def veryfi_int(cls, digit):
        """Проверка передаваемого параметра, что он является int числом"""
    ...
    def __set__(self, instance, value):
        """Setter"""
        self.veryfi_int(value)  # проверка на int
...
class TableValues:  # для работы с таблицей в целом.
    def __init__(self, rows, cols, cell=None):  # rows, cols - число строк и столбцов (целые числа);
        if cell is None:
            raise ValueError('параметр cell не указан')
        else:
            self.cells = tuple(tuple(cell() for i in range(cols)) for _ in range(rows))
