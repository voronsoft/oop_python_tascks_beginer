...
class Student:
    ...
    def set_mark(self, student, mark):
        raise NotImplementedError('В дочернем классе должен быть переопределён метод - set_mark(self, student, mark)')
...
class Lector(Mentor):  # для описания лекторов;
    ...
    def __init__(self, fio, subject):
        if type(fio) == str and type(subject) == str:
    ...
    # Также в классах Lector должен быть переопределен магический метод:
    # __str__()
    # для формирования следующей информации об объектах:
    # - для объектов класса Lector: Лектор <ФИО>: предмет <предмет>
    def __str__(self):
...
class Reviewer(Mentor):  # для описания экспертов.
    ...
    # Также в классах Reviewer должен быть переопределен магический метод:
    # __str__()
    # для формирования следующей информации об объектах:
    # - для объектов класса Reviewer: Эксперт <ФИО>: предмет <предмет>
    def __str__(self):
...
from test4_5.test_4_5_3 import test_3

test_3(Lector, Mentor, Reviewer, Student)
