...
class Array:
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, int) and 0 <= item < len(self.lst_obg_cell):
            return self.lst_obg_cell[item].value
        else:
            raise IndexError('неверный индекс для доступа к элементам массива')
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, int):
            if 0 <= key < len(self.lst_obg_cell):
                self.lst_obg_cell[key].value = value
            else:
                raise IndexError('неверный индекс для доступа к элементам массива')
        else:
            raise ValueError('должно быть целое число')
    ...
    # print(ar_int) - должны отображаться все значения массива в одну строчку через пробел
    def __str__(self):
        return f"{' '.join([str(_.value) for _ in self.lst_obg_cell])}"


class Integer:
    def __init__(self, start_value=0):  # Начальные значения в ячейках массива (в объектах Integer) должны быть равны 0.
        if isinstance(start_value, int):
            self.__value = start_value  # начальное значение ячейки (в данном случае - целое число).
        else:
            raise ValueError('должно быть целое число')
...
from test3_8.test_3_8_4 import test_4

test_4(Array, Integer)
